...
@section('title', 'Child Transit & Bus Routes | Parent Portal | EduLink')
@section('header_title', 'School Bus Transit')
...
<div class="container-fluid p-0">
    ...
    <div class="glass-card border-0 rounded-4 overflow-hidden mb-4 p-4 position-relative" style="background: linear-gradient(135deg, rgba(var(--bs-primary-rgb), 0.08) 0%, rgba(var(--bs-warning-rgb), 0.05) 100%); border-left: 5px solid var(--primary-color) !important;">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
            ...
            <div>
                <span class="badge bg-primary bg-opacity-10 text-primary px-3 py-2.5 rounded-3 border border-primary border-opacity-10"><i class="bi bi-geo-alt me-1"></i> Parent Directory</span>
            </div>
        </div>
    </div>

    <!-- Active Child Selector Widget -->
    @if($children->count() > 1)
        <div class="glass-card p-3 mb-4 border-0 shadow-sm">
            <div class="d-flex align-items-center gap-3 flex-wrap">
                <span class="fw-bold text-dark small"><i class="bi bi-people me-1"></i>Select Child Profile:</span>
                @foreach($children as $child)
                    <a href="{{ route('school.parent-portal.select-child', $child->id) }}" class="btn btn-sm @if($activeChild && $activeChild->id === $child->id) btn-primary @else btn-outline-secondary @endif rounded-3 px-3 py-1.5 small shadow-xs">
                        <i class="bi bi-person-fill me-1"></i>{{ $child->first_name }}
                    </a>
                @endforeach
            </div>
        </div>
