...
@section('title', 'Edit Global Role | EduLink')
...
<div class="container-fluid p-0">
    ...
    <div class="row mb-4">
        <div class="col-12">
            <h5 class="fw-bold mb-1 text-dark">Edit Global Role and Permissions</h5>
            <p class="text-muted small">Update role details and adjust administrative permission mappings.</p>
        </div>
    </div>

    <form action="{{ route('super-admin.roles.update', $role->id) }}" method="POST" id="roleForm">
        @csrf
        @method('PUT')
            ...
            <div class="col-lg-4">
                <div class="glass-card p-4 shadow-sm h-100">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control rounded-3" id="name" name="name" required placeholder="e.g. Platform Supervisor" value="{{ old('name', $role->name) }}">
                    </div>

                    <div class="mb-3">
                        <label for="slug" class="form-label small fw-semibold">Role Slug</label>
                        <input type="text" class="form-control rounded-3" id="slug" name="slug" required placeholder="e.g. platform-supervisor" value="{{ old('slug', $role->slug) }}" @if($role->is_system) readonly @endif>
                        <div class="form-text small text-muted">A unique URL-friendly identifier. @if($role->is_system) <span class="text-danger">System slugs cannot be renamed.</span> @endif</div>
                    </div>

                    <div class="mb-3">
                        <label for="description" class="form-label small fw-semibold">Description</label>
                        <textarea class="form-control rounded-3" id="description" name="description" rows="4" placeholder="Brief explanation of this role's responsibilities...">{{ old('description', $role->description) }}</textarea>
                    </div>
                    ...
                    <div class="mt-4 p-3 bg-light rounded-4">
                        ...
                        <span class="text-muted small" style="font-size: 0.75rem;">
                            Modifying a role immediately updates capabilities for all platform users assigned to this role.
                        </span>
                    </div>
                </div>
            </div>
                ...
                <div class="glass-card p-4 shadow-sm">
                                        ...
                                        <div class="col-md-6">
                                            <div class="p-2 bg-white rounded-3 border d-flex gap-3 align-items-start h-100">
                                                <div class="form-check pt-1 ps-3">
                                                    <input class="form-check-input permission-checkbox perm-{{ Str::slug($module) }}" type="checkbox" name="permissions[]" value="{{ $perm->id }}" id="perm_{{ $perm->id }}" @if($checked) checked @endif>
                                                </div>
                    ...
                    <div class="mt-4 pt-3 border-top text-end">
                        ...
                        <button type="submit" class="btn btn-primary rounded-3 px-4 py-2 fw-semibold">
                            <i class="bi bi-save me-1"></i>Update & Sync Permissions
                        </button>
                    </div>
                </div>
            </div>
...
<script>
    ...
    // Run initial setup check on load
    document.addEventListener("DOMContentLoaded", function() {
        updateSelectAllState();
        
        // Sync module header checkboxes based on child checks
        @foreach($permissions as $module => $modulePerms)
            (function() {
                const moduleSlug = '{{ Str::slug($module) }}';
                const total = document.querySelectorAll(`.perm-${moduleSlug}`).length;
                const checked = document.querySelectorAll(`.perm-${moduleSlug}:checked`).length;
                document.getElementById(`mod_${moduleSlug}`).checked = (total === checked && total > 0);
            })();
        @endforeach
    });
